...
usdaRead <- function(fruta = c("Blackberries", "Blueberries"), 
                     tipo = c("Precios",  "Movimientos")){
  
  readRDS(paste0(tipo,"/", fruta, ".RDS"))
...
movFit <- function(){
  ...
  frutas <- c("Blackberries", "Blueberries")
  
  movFit2 <- function(fruta){
    
    
    usda <- usdaRead(fruta, "Movimientos")%>%
      ddply(.(date),summarize, 
            Close = sum(as.numeric(totalWeightSum)))
    
    
    
    
    usdaXts <- xts(data.frame(Close = usda$Close),
                order.by = usda$date)%>%
      apply.weekly(sum)
    ...
    write_rds(usdaXts, paste0("mov", fruta, ".RDS"))
    
    data <-  data.frame(Fecha = index(usdaXts), Close = coredata(usdaXts))%>%
      tiempos("Semanats")
    
    frutaTs <- ts(data$Close, 
                  start = c(min(as.integer(format(data$Fecha, format = "%Y"))), 
                            min(data$Semanats)),
                  frequency = 52.25)
    
    fit <- tbats(frutaTs)
    
    write_rds(fit, paste0("fit", fruta,".RDS"))
  } 
...
entradasPag <- function(desde = Sys.Date(), hasta = Sys.Date(), 
                        Clave_productor = NULL, resumen = TRUE){
  ...
  entradas_fruta
    ...
    entradas_fruta%>%
      ddply(.(fecha, clave_producto), summarize, Total = sum(cantidad_aceptada))
  }
}
